...
//
// Il existe de [très nombreuses manières](https://github.com/markerikson/redux-ecosystem-links/blob/master/middleware-async.md)
// de faire de l’asynchrone avec Redux.  Ici, comme nous voulons
// illustrer des appels API et la capacité à résister à une interruption
// de connectivité au serveur, nous avons choisi le très sympathique
// module [redux-offline](https://github.com/jevakallio/redux-offline),
// lui-même une surcouche de l’excellent
// [redux-persist](https://github.com/rt2zz/redux-persist). Lorsque
// ce module est mis en place (voir [la configuration du *store*](./store.html)),
// on peut décorer des actions avec un champ `meta.offline`, lequel
// peut contenir des propriétés `effect` (qui décrit, par défaut, un
// appel API), `commit` (action à *dispatcher* en cas de réponse
// réseau à succès, donc codes 2xx) et `rollback` (action à *dispatcher*
// en cas de réponse réseau sans succès, donc codes de réponse autres,
// ex. 401).
//
// On a ici une action dite *pessimiste* : on doit la considérer
// échouée jusqu’à preuve du contraire (c’est un login, après tout).
// On finalise donc le login via l’action `LOGIN_SUCCESS`, dans le
// descripteur `commit`.  Le descripteur `rollback` déclenche une
// `LOGIN_FAILURE` qui permettra d’afficher une *snackbar* d’erreur
